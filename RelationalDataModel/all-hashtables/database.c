...
int main(int argc, char const *argv[]) {
  ...
  // delete_CPQ(cpq, "*", "*");
  // delete_CDH(cdh, "*","*","*");
  // delete_CR(cr, "*", "Turing Aud.");

  CPQ_RELATION l = lookup_CPQ(cpq,"EE200","*");
  printf("lookup:\n");
  print_CPQ(l);
  printf("\n");

  //csg->storage = unon(csg->storage, csg2->storage);
